...
class ManifestManager(object):
    ...
    def _compileParameters(self, params):
        ...
        # Each parameter must be a key / dictionary pair
        for param, attrs in params.iteritems():
            if not isinstance(attrs, dict):
                raise ManifestError('Parameter definition for {0} must be a dictionary mapping'.format(param))

            # Required key must be a bool if present
